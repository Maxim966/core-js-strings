...
function formatTime(minutes, seconds) {
  const minutesFotmat = minutes.toString().padStart(2, '0');
  const secondsFormat = seconds.toString().padStart(2, '0');
  return `${minutesFotmat}:${secondsFormat}`;
}
...
function countVowels(str) {
  const vowelsStr = 'aeiouyAEIOUY';
  let count = 0;
  for (let index = 0; index < str.length; index += 1) {
    if (vowelsStr.includes(str[index])) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  ...
  for (let index = 0; index < newStr.length; index += 1) {
    ...
    if (char >= 'a' && char <= 'z') {
      onlyLetters += newStr[index];
    }
  }

  const reversStr = onlyLetters.split('').reverse().join('');
  return onlyLetters === reversStr;
}
